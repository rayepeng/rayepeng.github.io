...
### 5.0a 阶段 — Markdown 渲染验证文档（2026-06-06 插入）

> 在继续任何内容迁移之前，创建一份全面的 Markdown 渲染验证文章，
> 覆盖所有主流 Markdown 元素和 Astro 管线特性，**重点关注图片渲染**。
> 所有渲染缺陷必须在本阶段修复完毕，后续迁移才能基于稳定的管线进行。
>
> **红线：严禁在本阶段迁移任何 antfu.me 原始文档。**

#### 任务清单

- [ ] 创建 `src/content/posts/markdown-render-test.mdx`，包含以下元素：
  - [ ] 标题层级 h1–h4 + `#` 锚点跳转
  - [ ] 段落 / 换行 / 空行分隔
  - [ ] **粗体** / *斜体* / ~~删除线~~ / `inline code` / [超链接](url)
  - [ ] 有序列表 / 无序列表 / 嵌套列表 / 任务列表 `- [x]`
  - [ ] 引用块 / 嵌套引用
  - [ ] 表格（含对齐）
  - [ ] 水平分割线 `<hr>`
  - [ ] 行内图片 `![](url)` — 使用 public 目录下的测试图片
  - [ ] 图片不同尺寸：小图 / 大图 / 超宽图（测试溢出与居中）
  - [ ] 带 alt 文本的图片（验证 figcaption 生成）
  - [ ] 带 title 的图片（悬停提示）
  - [ ] 图片链接（点击图片跳转）
  - [ ] 相对路径引用的图片（验证 public 目录映射）
  - [ ] 代码块（无语言标注 / ts / diff / 带 `[!code highlight]` / 带 `[!code ++]` `[!code --]`）
  - [ ] GitHub Alert: `> [!NOTE]` / `> [!TIP]` / `> [!WARNING]` / `> [!CAUTION]`
  - [ ] `<MagicLink>` 组件（Vue/Vite 等）
  - [ ] HTML 内嵌标签（`<br>` / `<sup>` / `<sub>` / `<details><summary>`）
- [ ] 在 `public/` 下准备 3 张不同尺寸的测试图片
- [ ] 通过 `pnpm build && pnpm preview` 逐项核对渲染输出
- [ ] 修复发现的渲染缺陷（如：图片溢出、figcaption 缺失、代码块主题不切换、锚点无效等）
